...
export class BlobStoreManager {
  ...
  async getUsers(): Promise<Record<string, any>> {
    try {
      ...
      console.log(' Donn�es utilisateurs charg�es via Blob Store');
  ...
  async getQuests(): Promise<Record<string, any>> {
    try {
      ...
      console.log(' Donn�es qu�tes charg�es via Blob Store');
  ...
  async getCommonQuests(): Promise<string[]> {
    try {
      ...
      console.log(' Qu�tes communes charg�es via Blob Store');
  ...
  async getAdminPassword(): Promise<string> {
    try {
      ...
      console.log(' Mot de passe admin charg� via Blob Store');
      return password;
    } catch (error) {
  ...
  async updateUsers(users: Record<string, any>): Promise<boolean> {
    try {
      ...
      console.log(' Utilisateurs mis � jour avec succ�s dans Blob Store:', blob.url);
      return true;
    } catch (error) {
      this.metrics.failedOperations++;
  ...
  async updateQuests(quests: Record<string, any>): Promise<boolean> {
    try {
      ...
      console.log(' Qu�tes mises � jour avec succ�s dans Blob Store:', blob.url);
      return true;
    } catch (error) {
      this.metrics.failedOperations++;
  ...
  async createBackup(config?: BlobStoreConfig): Promise<string | null> {
    try {
      ...
      console.log(' Backup cr�� avec succ�s:', blob.url);
      return blob.url;
    } catch (error) {
  ...
  async updateFullConfig(config: BlobStoreConfig): Promise<boolean> {
    try {
      ...
      console.log(' Configuration mise � jour avec succ�s dans Blob Store:', blob.url);
      return true;
    } catch (error) {
      this.metrics.failedOperations++;
  ...
  async cleanupOldBackups(keepCount: number = 10): Promise<boolean> {
    try {
      ...
      console.log(` Nettoy� ${blobsToDelete.length} anciens backups`);
      return true;
    } catch (error) {
